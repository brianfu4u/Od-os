...
async function main(): Promise<void> {
  ...
  try {
    ...
    check(!!r1 && Math.abs(r1.verificationScore - 0.5) < 0.01, `verificationScore ≈ 0.50 (got ${r1?.verificationScore})`);
    ...
    check(Math.abs(Number(afterVerify2.rows[0]!.verification_score) - 0.855) < 0.01, `verification_score ≈ 0.855 (got ${afterVerify2.rows[0]!.verification_score})`);

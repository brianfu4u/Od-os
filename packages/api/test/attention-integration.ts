...
async function insClaim(
  ...
  status: string,
  claimedAtIso: string,
  verification?: { result?: string; verificationScore?: number },
): Promise<void> {
  const claim = await admin.query<{ id: string }>(
    `INSERT INTO employee_status_claims (tenant_id, employee_id, claimed_status, claim_source, claimed_at)
     VALUES ($1,$2,$3,'button',$4) RETURNING id`,
    [tenant, employeeId, status, claimedAtIso],
  );
  if (verification?.result && verification.verificationScore !== undefined) {
    await admin.query(
      `INSERT INTO employee_status_verification_ledger
         (tenant_id, claim_id, employee_id, verification_result, verification_score)
       VALUES ($1,$2,$3,$4,$5)`,
      [tenant, claim.rows[0]!.id, employeeId, verification.result, verification.verificationScore],
    );
  }
}
...
async function main(): Promise<void> {
  ...
  try {
    console.log(
      'T-06/T-07/T-10 attention — generation, audit-no-dedup, display-dedup, dequeue, isolation:',
    );

    // Silent employee in tenant A: on_duty, last valid event 2h ago (> 3600s silence threshold).
    const silent = await insObject(
      admin,
      A,
      'Staff',
      { displayName: 'Silent A', staffHandle: 'silent_a' },
      'on_duty',
    );
    ...
    const active = await insObject(
      admin,
      A,
      'Staff',
      { displayName: 'Active A', staffHandle: 'active_a' },
      'on_duty',
    );
    ...
    const silentB = await insObject(
      admin,
      B,
      'Staff',
      { displayName: 'Silent B', staffHandle: 'silent_b' },
      'on_duty',
    );
    ...
    check(
      silenceItems.length === 1,
      'generation: exactly one silence item for the silent on_duty employee',
    );
    ...
    check(
      silenceItems[0]?.evidenceSummary.claimed === 'on_duty',
      'evidence: states the claimed fact (neutral, no verdict)',
    );
    check(
      !view1.items.some((i) => i.employeeId === active),
      'generation: the active employee does NOT surface',
    );

    // ── isolation ──
    check(
      !view1.items.some((i) => i.employeeId === silentB),
      'isolation: tenant B silent staff never appears in tenant A queue',
    );
    ...
    check(
      Number(auditCount.rows[0]!.n) === 0,
      'P1-5: N reads → 0 attention.candidate.generated events (read path writes nothing)',
    );

    // ── never touches world state: NO event of ANY kind is written by reading the queue ──
    const staffRow = await admin.query<{ claimed_state: string | null }>(
      `SELECT claimed_state FROM objects WHERE id = $1`,
      [silent],
    );
    check(
      staffRow.rows[0]!.claimed_state === 'on_duty',
      'world-state: generation does NOT change claimed_state',
    );
    const anyWrite = await admin.query<{ n: string }>(
      `SELECT count(*) AS n FROM events WHERE object_id = $1 AND event_type LIKE 'attention.%'`,
      [silent],
    );
    check(
      Number(anyWrite.rows[0]!.n) === 0,
      'world-state: reading the queue produces NO attention.* event at all',
    );
    const empVisible = await admin.query<{ n: string }>(
      `SELECT count(*) AS n FROM events WHERE object_id = $1 AND actor = 'employee'`,
      [silent],
    );
    check(
      Number(empVisible.rows[0]!.n) === 0,
      'world-state: generation produces NO employee-actor event',
    );
    ...
    check(
      !view3.items.some((i) => i.employeeId === silent && i.kind === 'silence'),
    ...
    const lowc = await insObject(
      admin,
      A,
      'Staff',
      { displayName: 'LowC A', staffHandle: 'lowc_a' },
      'idle',
    );
    await insClaim(admin, A, lowc, 'idle', iso(60), {
      result: 'inconsistent',
    ...
    check(
      view4.items.some((i) => i.employeeId === lowc && i.kind === 'low_confidence'),
      'low_confidence: an inconsistent verification verdict surfaces a manager-side item',
    );

    const boundary59 = await insObject(
      admin,
      A,
      'Staff',
      { displayName: 'B59', staffHandle: 'b59' },
      'idle',
    );
    await insClaim(admin, A, boundary59, 'idle', iso(40), {
      result: 'consistent',
      verificationScore: 0.59,
    });
    const boundary60 = await insObject(
      admin,
      A,
      'Staff',
      { displayName: 'B60', staffHandle: 'b60' },
      'idle',
    );
    await insClaim(admin, A, boundary60, 'idle', iso(30), {
      result: 'consistent',
      verificationScore: 0.6,
    });
    const boundaryView = await service.queue(A);
    check(
      boundaryView.items.some((i) => i.employeeId === boundary59 && i.kind === 'low_confidence'),
      'boundary: verificationScore 0.59 enters attention queue',
    );
    check(
      !boundaryView.items.some((i) => i.employeeId === boundary60 && i.kind === 'low_confidence'),
      'boundary: verificationScore 0.60 does not enter attention queue',
    );
  } finally {
    await admin.end();
    await closePool();

...
export interface VerificationResult {
  ...
  /**
   * Deterministic S2 verdict score in [0,1]. A RULE score, NOT a statistical probability
   * (renamed from `confidence` in P1-4 to stop over-trust). Distinct from any LLM/STT confidence.
   */
 ...
 *    strength before it is folded into the verificationScore. 1.0 = neutral (pre-S0-7 behavior).
 *    Lets a task type say "a snapshot is worth more than a document" for THIS task.
 *  - `baseSelfClaim`: the base verificationScore a lone, matching self-claim carries BEFORE any
